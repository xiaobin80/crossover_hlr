...
begin
  ...
  adodataset_login.CommandText:='select OperID, OperPassWord from operator where OperID='
                                +''+edit_user.Text+''
                                +' and OperPassWord='+''''+edit_pass.Text+'''';
  ...
  begin
    ...
    preserve1.Close;
    preserve1.SQL.Clear;
    preserve1.SQL.Text:='select OperID, OperPassWord from operator where OperID='
                                        +''+edit_user.Text+''+' and preserve1=1';
    preserve1.Open;
    if preserve1.RecordCount<>0 then
    begin
      Application.MessageBox('用户已经在操作中，你不能登录！','提示',
    ...
    preserve1.Close;
    preserve1.SQL.Clear;
    preserve1.SQL.Text:='update operator set preserve1=1 where OperID='
                                                        +''+edit_user.Text+'';
    preserve1.ExecSQL;
    Close;
  end
...
begin
  //
  try
    CDP_INI:=TIniFile.Create(ExtractFilePath(ParamStr(0))+'CDPconfig.ini');
    xbf:=ExtractFilePath(ParamStr(0))+CDP_INI.ReadString('file name','1','zlnr1.xbf');
  finally
    CDP_INI.Free;
  end;
  //
